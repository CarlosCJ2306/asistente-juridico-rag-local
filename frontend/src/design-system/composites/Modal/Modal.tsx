...
export type ModalProps = ModalBaseProps & ModalDismissPolicy;

const sizeClasses = { sm: styles.dialogSm, md: styles.dialogMd, lg: styles.dialogLg } as const;

export function Modal({ open, onOpenChange, title, description, children, footer, size = "md", preventClose = false, preventCloseReason, closeLabel = "Cerrar diálogo", className }: ModalProps) {
  const titleId = useId();
  ...
  return (
    <dialog ref={dialogRef} aria-labelledby={titleId} aria-describedby={describedBy} onCancel={handleCancel} onClose={handleClose} onClick={handleBackdropClick} className={classNames(styles.dialog, sizeClasses[size], className)}>
